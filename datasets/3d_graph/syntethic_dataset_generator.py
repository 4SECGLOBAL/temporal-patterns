...
for buckets_it in tqdm(range(58000, 102000, 2000), desc='Execução 2'): #400
  for itens_it in range(400,1000,50): #100000
    ...
    # Aplicar o apriori 
    start_time = time.time()
    results =  fim.arules(buckets,supp=-int(3), conf=70, report='ab' , zmin=2)
    end_time = time.time()

    # Registrar a quantidade de 'itens', 'buckets' e o tempo de execução (Unico a fazer em arquivo)
    print("Itens: {} | Buckets: {} | Tempo Arules: {}".format(itens_it, buckets_it, end_time - start_time))
    # Registrar o resultado em um arquivo           
    with open('resultsArules.txt', 'a') as f:
      f.write("{},{},{}\n".format(itens_it, buckets_it, end_time - start_time))
      f.close()

    start_time = time.time()
    results = fim.apriori(buckets,supp=-int(3), conf=70, report='ab' , zmin=2, target='r')
    end_time = time.time()

    print("Itens: {} | Buckets: {} | Tempo Apriori: {}".format(itens_it, buckets_it, end_time - start_time))
    with open('resultsApriori.txt', 'a') as f:
      f.write("{},{},{}\n".format(itens_it, buckets_it, end_time - start_time))
      f.close()
    ...
    start_time = time.time()
    results = fim.fpgrowth(buckets,supp=-int(3), conf=70, report='ab' , zmin=2, target='r')
    end_time = time.time()

    print("Itens: {} | Buckets: {} | Tempo Fpgrowth: {}".format(itens_it, buckets_it, end_time - start_time))
    with open('resultsFpgrowth.txt', 'a') as f:
      f.write("{},{},{}\n".format(itens_it, buckets_it, end_time - start_time))
      f.close()
