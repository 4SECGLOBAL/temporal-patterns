...
#     print(f"Dataset com {num_patterns} padrões aleatórios (total {total_records} registros) salvo em '{output_csv}'")
...
def insert_defined_patterns(input_csv, output_txt, patterns_list, max_time_between_parts):
    ...
    for pattern_info in patterns_list:
        ...
        # Criação da base e dos rastreios
        for rep in range(reps):
            random_seconds = random.randint(0, int(time_range))
            base_time = min_time + timedelta(seconds=random_seconds)
            rep_timestamps = []
            
            # Tarefa I: Criar um registro para cada parte do padrão
            for part in pattern_parts:
                try:
                    if '=>' not in part:
                        continue
