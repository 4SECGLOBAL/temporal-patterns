...
#         else:
#             clusters_copy = {rule['cluster'] for rule in filteredCopy}
#             clusters_rules = {rule['cluster'] for rule in filtered_rules}
#             if len(clusters_copy) != len(clusters_rules):
#                 filtered_rules = filteredCopy
                
                
                
        else:
            cluster_counts = Counter(rule['cluster'] for rule in filteredCopy)
            dominant_clusters = [c for c, count in cluster_counts.items() if count >= 3]
            
            # Substitui se pelo menos um cluster aparece 3+ vezes E ele é maioria
            if dominant_clusters and len(dominant_clusters) / len(cluster_counts) >= 0.7:
                filtered_rules = filteredCopy.copy()
...
def main(k, n, max_items, repetitions, eps, tempo):
    input_file_path = n
