...
def handle_outliers(data, input_file_path, eps=None, min_samples=None, plot=True):
    ...
    #	VARIAÇÃO COM HDBSCAN -> grande quantidade de amostras
...
#     input_file_path = "0012.csv"
    input_file_path = n
#     min_repeticao = int(input("Qual o numero de repeticoes por padrao? \n"))
...
#     data['cluster'] = apply_dbscan(data_scaled)
    ...
    clean_data, _ = handle_outliers(data, input_file_path, eps)
...
#     auto_min_support = max(0.0001, item_counts.max() / len(transactions) * 0.5)
