...
def handle_outliers(data, input_file_path, eps, min_samples, plot=True):
    ...
    original_index = data_clean.index
    
    def safe_conversion(value):
        if pd.isna(value):
            # Valor especial para NaN (fora do range normal)
            return -999999  
        if isinstance(value, str):
            if value.isdigit():
                return float(value)
            else:
                return float(abs(hash(value)) % 10**8)  
        return float(value)
    
    # Conversão I:
    data_clean['origem_num'] = data_clean['origem'].apply(safe_conversion)
    data_clean['destino_num'] = data_clean['destino'].apply(safe_conversion)
    
    # Normalização I: segundos
    try:
        timestamps = pd.to_datetime(data_clean['timestamp'], errors='coerce')
        start_time = timestamps.min()
        data_clean['timestamp_num'] = (timestamps - start_time).dt.total_seconds()

        data_clean['timestamp_num'] = data_clean['timestamp_num'].fillna(-1)
    except Exception as e:
        print(f"Erro no processamento do timestamp: {e}")
        data_clean['timestamp_num'] = 0 
    
    features = data_clean[['timestamp_num', 'origem_num', 'destino_num']].copy()
    ...
    outliers_mask = data_clean['cluster'] == -1
    ...
    if plot:
        try:
            ...
            ax.scatter(
                clean_data.loc[~outliers_mask, 'origem_num'],
                clean_data.loc[~outliers_mask, 'destino_num'],
                clean_data.loc[~outliers_mask, 'timestamp_num'],
                c=clean_data.loc[~outliers_mask, 'cluster'],
                cmap='viridis',
                label='Dados Normais'
            )
            ...
            ax.set_xlabel('Origem (numérico)')
            ax.set_ylabel('Destino (numérico)')
            ax.set_zlabel('Timestamp (segundos)')
            ax.set_title(f'Visualização de Clusters - Outliers: {outliers_mask.sum()}/{len(data)}')
            plt.legend()
            plt.tight_layout()
            
            output_img = os.path.splitext(input_file_path)[0] + "_clusters.png"
            plt.savefig(output_img)
            plt.close(fig)
            figs.append(fig)
            print(f"Gráfico de clusters salvo como: {output_img}")
        except Exception as e:
            print(f"Erro na geração do gráfico: {e}")
    
    final_data = data_clean.drop(columns=['timestamp_num', 'origem_num', 'destino_num'])
    clean_data = clean_data.drop(columns=['timestamp_num', 'origem_num', 'destino_num', 'cluster'])
    
    return final_data, figs
...
def preprocess_for_dbscan(data):
    ...
    # Conversão I:
    data['origem_num'] = data['origem'].apply(convert_to_numeric)
    data['destino_num'] = data['destino'].apply(convert_to_numeric)
    
    data['timestamp_num'] = pd.to_datetime(data['timestamp']).astype(int) / 10**9
    features = data[['timestamp_num', 'origem_num', 'destino_num']]
    scaler = StandardScaler()
...
def group_patterns_by_time(rules, clean_data, eps):
    ...
    for _, row in rules.iterrows():
        ...
        timestamps = clean_data[
            (clean_data['origem'] == origem) & 
            (clean_data['destino'] == destino)
        ]['timestamp'].tolist()
        ...
        if groups:
            ...
            for i, group in enumerate(filtered_groups, start=1):
                ...
                for rt in group:
                    ...
                    timestamps = clean_data[(clean_data['origem'] == origem) & 
                                          (clean_data['destino'] == destino)]['timestamp'].tolist()
